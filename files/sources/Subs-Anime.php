 ...
 * modifySeriesMessage()                                							*
 * getAnimeSagasInfo()                                  							*
 * getAnimeSagasInfoForSeries()                         							*
 * modifyAnimeSeriesBoard()                             							*
 * generateSagasMessage()                               							*
 * generateDownloadsForSaga()                           							*
 * createSagasMessage()                                 							*
 * modifySagasMessage()                                 							*
 * addSagas()                                           							*
 * modifySagas()                                        							*
 ...
 * modifyServer()                                       							*
 * updateAllSagas()                                     							*
 * getAnimeUnpublishedChaptersInfo()                    							*
 * getAnimeChaptersInfo()                               							*
 * modifyChapterLinks()                                 							*
 * getAnimeCapturesForChapter()                         							*
 ...
 * addChapter()                                         							*
 * modifyChapter()                                      							*
 * generateChapterMessage()                             							*
 * addChapterMessage()                                  							*
 * modifyChapterMessage()                               							*
 * modifyChapterCaptures()                              							*
 * addCapture()                                         							*
 * modifyCapture()                                      							*
 * addLink()                                            							*
 * modifyLink()                                         							*
 * getAnimeNewsInfo()                                   							*
 * getAnimeChaptersForNews()                            							*
 * generateNewsSubject()                                							*
 * generateAnimeGallery()                               							*
 * generateNewsMessage()                                							*
 * createNewsMessage()                                  							*
 * modifyNewsMessage()                                  							*
 * getSelectedChapters()                                							*
 ...
 * regenerateAnimeNews()                                							*
 * deleteAnimeNews()                                    							*
 * regenerateAnimeChapter()                             							*
 * deleteCapture()                                      							*
 * deleteChapterLinks()                                 							*
 * deleteAnimeChapter()                                 							*
 * regenerateAnimeSagas()                               							*
 * deleteAnimeSagas()                                   							*
 * regenerateAnimeSeries()                              							*
 * deleteAnimeSeries()                                  							*
 * getAnimeSpecsInfo()                                  							*
 * ********************************************************************************	*/
...
function getAnimeSeriesInfo($series_id = null, $category_id = null, $limit = true) {
...
function editSeries($seriesInfo = null) {
	...
	$series_fields[] = "Specs = {string:Specs}";
...
function getFirstMessage($topic_id = null) {
	global $scripturl, $context, $smcFunc, $settings, $txt, $user_info, $sourcedir;
...
function generateSeriesMessage($seriesInfo) {
	...
	$message .= "[center][font=trebuchet ms][size=10pt]" . $seriesInfo['specs'] . $nl . $nl;
	$message .= $seriesInfo['staff'] . "[/size][/font][/center]" . $nl . $nl;
...
function createSeriesBoard($seriesInfo = null) {
	...
	if ($seriesInfo['finalized']) {
		...
		if ($seriesInfo['public']) {
			...
			$boardOptions['access_groups'] = $aux;
...
function createSeriesMessage($seriesInfo) {
	...
	//setAnimeManagerPost($msgOptions); 
...
function getAnimeSagasInfo($sagaId = null, $serieId = null, $limit = true) {
	...
	$request = $smcFunc['db_query']('', '
		SELECT ser.Name as SName, sag.ID_SAGA, sag.ID_SERIES, sag.ID_MSG, sag.Name as Name, sag.Image as Image, sag.Abstract, sag.Staff, sag.Specs, sag.Licensed, ser.Img_Banner as sImage, ser.ID_MSG as SerMSG
...
function modifyAnimeSeriesBoard($seriesInfo = null) {
	global $context, $txt, $boards, $sourcedir;
...
function generateSagasMessage($seriesInfo, $sagasInfo) {
	...
	$message .= "[center][font=trebuchet ms][size=11pt]" . (empty($sagasInfo['specs']) ? $seriesInfo['specs'] : $sagasInfo['specs']) . $nl . $nl;
	$message .= (empty($sagasInfo['staff']) ? $seriesInfo['staff'] : $sagasInfo['staff']) . "[/size][/font][/center]" . $nl . $nl;
...
function modifySagas($sagasInfo = null) {
	...
	$sagasfields[] = "Specs = {string:specs}";
...
function findOrphanSeries() {
	$seriesInfo = getAnimeSeriesInfo(null,null,false);
...
function generateChapterMessage($chapterInfo = null) {
	...
	$message .= '[html]<center><a href="'.$scripturl.'?action=admin;area=an-chapters;sa=editChapters;chapter_id='.$chapterInfo['id'].';edit_chapter=1" ><button type="button" class="catbg" style="width:120px; height:30px; border-radius:7px">'.$txt['an-adminEditChapters'].'</button></a></center>[/html]';
	return $message;
}
...
function getAnimeNewsInfo($newsId = null, $last = true) {
	...
	$request = $smcFunc['db_query']('', '
		...
		'.(($last == true)?'ORDER BY n.ID_DOWNLOAD DESC':'ORDER BY id_category,Serie,id_saga,n.ID_DOWNLOAD').'
...
function generateBlockSeries($finalized = true) {
	...
	foreach ($categories as $cat) {
		...
		$series = getAnimeSeriesInfo(null, $cat['id'],false);
		...
		foreach ($series as $ser) {
			if ($ser['finalized'] == $finalized && $ser['public'] == true) {
				...
				$message .= '<div class="series_block"><a href="' . $ser['boardLink'] . '"><font color="grey">'.$seriesName.'</font>'.$nl.'<img src="' . $ser['img_download_block']['href'] . '" alt="'.$ser['name'].'" title="'.$ser['name'].'"/></a></div>';
			}
		}
	}
...
function setAnimeManagerPost($opts = null) {
	global $smcFunc;
	
	if (empty($opts)) {
		trigger_error("setAnimeManagerPost(): ID empty.");
	} else {
		$smcFunc['db_query']('', '
