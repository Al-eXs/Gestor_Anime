...
function anime_admin_unpublished_chapters_list() {
	...
	// Construct the page index. 50 articles per page.
	$context['page_index'] = constructPageIndex($scripturl . '?action=admin;area=an-chapters;sa=listUnpublishedChapters', $_REQUEST['start'], $context['total_articles'], 50);
...
function anime_admin_chapters_list() {
	...
	$smcFunc['db_free_result']($request);
	// Construct the page index. 50 articles per page.
	$context['page_index'] = constructPageIndex($scripturl . '?action=admin;area=an-chapters;sa=listChapters', $_REQUEST['start'], $context['total_articles'], 50);
	$context['start'] = $_REQUEST['start'];
	...
	if (empty($_POST['edit_chapter'])) {
		if (empty($_POST['category_id'])) {
			...
			if (empty($_POST['series_id'])) {
				$context['series'] = getAnimeSeriesInfo(null, $_POST['category_id'], false);
		...
		if (empty($_POST['category_id'])) {
			...
			if (empty($_POST['series_id'])) {
				...
				if (empty($_POST['sagas_id'])) {
					...
					//Heading to last step, fill the general information
					// Just we need the template.
					$_POST['chapter_id'] = (int) $_POST['chapter_id'];
					$_POST['category_id'] = (int) $_POST['category_id'];
					$_POST['series_id'] = (int) $_POST['series_id'];
