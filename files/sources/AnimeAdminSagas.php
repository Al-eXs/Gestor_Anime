...
function anime_admin_sagas_list() {
	...
	$context['page_index'] = constructPageIndex($scripturl . '?action=admin;area=an-sagas;sa=listSagas', $_REQUEST['start'], $context['total_articles'], 50);
...
function anime_admin_sagas_add()
{
	...
	if (empty($_POST['edit_sagas'])) {
		$context['seriesInfo'] = getAnimeSeriesInfo(null,null,false);
	...
	if (empty($_POST['sagas_name']) || empty($_POST['seriesSelect']))
		...
		if(!empty($_POST['sagas_specs']))
			$specs = $smcFunc['htmlspecialchars']($_POST['sagas_specs'], ENT_QUOTES);
		...
		$aux = getAnimeSeriesInfo($_POST['seriesSelect'],null,false);
		$seriesInfo = $aux[0];
...
function anime_admin_sagas_edit() {
	...
	if (empty($_POST['sagas_name']) || empty($_POST['seriesSelect']) || empty($_POST['saga_id']))
		...
		$sagasInfo['specs'] = (!empty($staff) ? $specs : '');
